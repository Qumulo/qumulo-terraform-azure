...
With `create_private_endpoints = true`, one `terraform apply` deploys the cluster and takes its
control-plane services private-ready. The qumulo provider deploys with public network access **on**
for the storage accounts, Key Vault, and App Configuration store (so the deployment never depends
on private-name resolution) and creates the private endpoints -- one per storage account, one for
the Key Vault, one for the App Configuration store -- as its final step, with no DNS attached,
reporting them in the `storage_private_endpoints`, `keyvault_private_endpoint`, and
`appconfig_private_endpoint` outputs. In the same apply, after the cluster completes, the wrapper:
...
Until the qumulo provider release that ships the endpoint outputs, supply their values via the
`private_endpoints_override` variable, which mirrors the outputs' shape (see
`examples/azure-private-link.tf`).
...
| <a name="input_blob_private_dns_zone_id"></a> [blob\_private\_dns\_zone\_id](#input\_blob\_private\_dns\_zone\_id) | OPTIONAL: Full resource ID of the `privatelink.blob.core.windows.net` (public) / `privatelink.blob.core.usgovcloudapi.net` (US Gov) private DNS zone; the wrapper creates one A record per storage-account private endpoint. Requires `create_private_endpoints`; same subscription as the Key Vault zone. Leave null on the external-DNS (e.g. Infoblox) path. | `string` | `null` | no |
...
| <a name="input_create_private_endpoints"></a> [create\_private\_endpoints](#input\_create\_private\_endpoints) | OPTIONAL: Enable the private-endpoint deployment mode -- see [Private networking (post-deployment)](#private-networking-post-deployment). Conflicts with the LEGACY `private_link_*_dns_zone_id` / `disable_*_public_network_access` variables. | `bool` | `false` | no |
...
| <a name="input_disable_public_network_access_post_deploy"></a> [disable\_public\_network\_access\_post\_deploy](#input\_disable\_public\_network\_access\_post\_deploy) | OPTIONAL: Disable public access on the storage accounts, Key Vault, and App Configuration store as the last step of the apply. Azure DNS path only (requires `create_private_endpoints` and the zone IDs); external-DNS callers lock down from their own tooling via the `private_endpoints` output. | `bool` | `false` | no |
...
| <a name="input_keyvault_private_dns_zone_id"></a> [keyvault\_private\_dns\_zone\_id](#input\_keyvault\_private\_dns\_zone\_id) | OPTIONAL: Full resource ID of the `privatelink.vaultcore.azure.net` (public) / `privatelink.vaultcore.usgovcloudapi.net` (US Gov) private DNS zone; the wrapper creates the Key Vault endpoint's A record. Requires `create_private_endpoints`; same subscription as the blob zone. Leave null on the external-DNS path. | `string` | `null` | no |
...
| <a name="input_private_endpoints_override"></a> [private\_endpoints\_override](#input\_private\_endpoints\_override) | INTERIM: The private-endpoint details the qumulo provider created, mirroring its upcoming `appconfig_private_endpoint` / `keyvault_private_endpoint` / `storage_private_endpoints` outputs; goes away when that release ships. | `object({ appconfig = optional(object), keyvault = optional(object), storage = optional(map(object), {}) })` | `{ storage = {} }` | no |
| <a name="input_private_link_appconfig_dns_zone_id"></a> [private\_link\_appconfig\_dns\_zone\_id](#input\_private\_link\_appconfig\_dns\_zone\_id) | LEGACY: Resource ID of the privatelink.azconfig.io private DNS zone; the provider creates the App Configuration private endpoint bound to it during deployment. Superseded by `create_private_endpoints`. | `string` | `null` | no |
