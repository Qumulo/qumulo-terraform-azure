...
| <a name="input_running_cluster_version"></a> [running\_cluster\_version](#input\_running\_cluster\_version) | OPTIONAL: Qumulo Core version the cluster runs now, when in-cluster upgrades have moved it past the recorded cluster\_version (which never changes after create or import). Read from the cluster UI or `GET /v1/version`. Consulted only by the Azure v4 VM guard, which refuses a v4 vm\_type below Core 7.8.0.1. | `string` | `null` | no |
