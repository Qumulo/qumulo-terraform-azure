...
| <a name="input_node_replacement_when_changed"></a> [node\_replacement\_when\_changed](#input\_node\_replacement\_when\_changed) | OPTIONAL: A pure change-trigger, not a real setting -- the value itself is never inspected. Edit it to any new value (e.g. today's date, a short note) with no other change to force a full cluster replace: every node rebuilt one at a time, same `vm_type` and `node_count`. Use it to pick up a disk-layout or tunable fix that only applies to newly-built nodes. Requires a provider version that ships `node_replacement_when_changed` on `qumulo_filesystem_azure`. | `string` | `null` | no |
